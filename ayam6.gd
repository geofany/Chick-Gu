	...
	if status != "Mati":
		if GlobalVar.makanan == true && status != "Sakit":
			...
			if GlobalVar.makan == 0:
				get_node("/root/Level 6/MakananTimer").start(5)
				get_node("/root/Level 6/Tools/makanan/makan").play("cooldown")
			HP += 20
		if GlobalVar.minuman == true && status != "Sakit":
			GlobalVar.minum -= 1
			if GlobalVar.minum == 0:
				get_node("/root/Level 6/MinumanTimer").start(5)
				get_node("/root/Level 6/Tools/Air/air").play("cooldown")
			HP += 10
		if GlobalVar.vaksin == true && status == "Sakit":
			GlobalVar.vaks -= 1
			if GlobalVar.vaks == 0:
				get_node("/root/Level 6/VaksinTimer").start(5)
				get_node("/root/Level 6/Tools/vaksin/Vaksin").play("cooldown")
			status = "Normal"
			if HP > 50:
				HP = 50
